...
namespace AccessoriesShop.Infrastructure.Services
{
    public class GroqService : IAIProvider
    {
        ...
        public async Task<bool> IsAvailableAsync() => !string.IsNullOrWhiteSpace(_settings.ApiKey);
...
//                    ? "Bạn là trợ lý phân tích sản phẩm phụ kiện. Hãy phân tích hình ảnh và trả về thông tin dưới dạng JSON."
...
//        }
        ...
        public bool SkillSupports(AICapability capability)
        {
            return capability == AICapability.TextGeneration;
        }

        public async Task<string> ExecuteAsync(AIRequest request, CancellationToken ct = default)
        {
            var systemPrompt = "Bạn là trợ lý tư vấn của cửa hàng phụ kiện điện thoại. Hãy trả lời thân thiện, ngắn gọn, và hữu ích bằng ngôn ngữ mà khách hàng đang dùng.";
